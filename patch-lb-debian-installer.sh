...
# works with trixie: lb_binary_debian-installer hardcodes the bootloader
# package names "lilo grub grub-pc" and "linux-image-2.6-amd64", none of
# which exist in trixie. Replace them with the bootloaders the Debian
# Installer actually needs from the ISO pool (grub-pc for BIOS, grub-efi-amd64
# for UEFI). grub-pc and grub-efi-amd64 conflict, so grub-efi-amd64 is fetched
# in a separate apt invocation. Also supplies the debian-cd data file that
# lb_binary_disk requires for releases newer than sid (Ubuntu ships data only
# up to sid). Idempotent; safe to run on every build.
set -euo pipefail
